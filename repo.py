...
def kv_set(key: str, val: str):
    with get_session() as s:
        s.execute(
            text("INSERT INTO config_kv (k, val) VALUES (:k, :v) ON DUPLICATE KEY UPDATE val=:v"),
            {"k": key, "v": val}
        )
        s.commit()
...
def log_error(source: str, message: str):
    with get_session() as s:
        s.execute(
            text("INSERT INTO error_log (source, message, created_at) VALUES (:s, :m, NOW())"),
            {"s": source, "m": message}
        )
        s.commit()
...
def schedule_due_now(limit: int = 5):
    with get_session() as s:
        return s.execute(text("""
...
def schedule_cancel_by_short_id(short_id: str) -> int:
    with get_session() as s:
        res = s.execute(text("""
            ...
            WHERE short_id=:sid AND status='QUEUED'
        """), {"sid": short_id})
        s.commit()
        return res.rowcount
...
def schedule_cancel_all_today() -> int:
    with get_session() as s:
        res = s.execute(text("""
            UPDATE scheduled_messages
            SET status='CANCELLED'
            WHERE status='QUEUED' AND DATE(send_at)=CURDATE()
        """))
        s.commit()
        return res.rowcount
...
def emit_count(kind: str, day):
    with get_session() as s:
        row = s.execute(text("""
            SELECT COUNT(*) FROM emit_log
            WHERE kind=:k AND DATE(created_at)=:d
        """), {"k": kind, "d": day}).fetchone()
        return int(row[0] or 0)

def emit_mark(kind: str):
    with get_session() as s:
        s.execute(text("INSERT INTO emit_log (kind, created_at) VALUES (:k, NOW())"), {"k": kind})
        s.commit()
