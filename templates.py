...
# Fallback minimalista per sicurezza
_FALLBACK = {
    "cta_link": "👉 {link}",
    "emojis_gasanti": ["🔥", "🚀", "⚡", "💎", "🏆", "🎯", "💥", "🎉", "💪", "🧨"],
    "value_single": {
        "title": "🔎 <b>VALUE SCANNER</b>",
        "outro_pool": ["Andiamo a prendercela."]
    },
    "multipla": {
        "title_map": {
            "2": "🧩 <b>DOPPIA</b> 🧩",
    ...
    "live_alert": {
        ...
        "body": "⏱️ {minute}’ — la favorita <b>{fav}</b> è sotto contro {other} (pre @ {preodd}).\nQuota live: {odds_str}",
        "outro_pool": ["Situazione perfetta per rientrare."]
    },
    "live_energy": {
    ...
    "almost_win": {
        "title_pool": ["PER UN SOFFIO"],
        "body": "Saltata per: {missed_leg}",
        "motivation_pool": ["Non preoccupatevi, la prossima volta sarà nostra. 💪🔥"]
    },
    "heartbreak": {
        "title": "💔 <b>CUORI SPEZZATI</b>",
        "line_pool": ["Scivolata a tempo scaduto: testa alta, ripartiamo. 🚀"]
    },
    "stat_flash": {
        "wrap": "📊 <b>STATISTICA LAMPO</b>\n\n{line}",
        "phrase_pool": ["Linea coerente con la scelta, numeri dalla nostra parte."]
    },
    "story": {
        "title_pool": ["Il colpo facile"],
        "long_body_pool": ["La tradizione dice equilibrio, ma i numeri raccontano altro. La nostra scelta è chiara. 🔥"]
    },
    "banter": {
        "pool": ["La value oggi è tutta dalla nostra parte. 🚀"]
    }
}

# Mappatura file frasi → chiavi nel JSON
_PHRASES_MAPPING = {
...
# --------------------------------------------------------------------
# Loader con merge delle frasi esterne (phrasi/*.txt) nei pool template
# --------------------------------------------------------------------
    ...
    for fname, targets in _PHRASES_MAPPING.items():
        ...
        for target in targets:
            ...
            if not isinstance(arr, list):
                arr = []
            # merge (unique) mantenendo l'ordine (set non preserva ordine in py<3.7)
            seen = set(arr)
            for ln in lines:
                if ln not in seen:
                    arr.append(ln)
...
def _load():
    try:
        ...
        data = dict(_FALLBACK)  # fallback profondo minimo
        _merge_phrases(data)
        _cache["data"] = data
        _cache["ts"] = time.time()
...
def render_multipla(legs: list, total_odds: float, kickoff: str, link: str) -> str:
    ...
    body_lines = [
        leg_tpl.format(home=e["home"], away=e["away"], pick=e["pick"], odds=float(e["odds"]))
        for e in legs
    ]
    body = "\n".join(body_lines)
...
def render_progress_bar(taken: int, total: int) -> str:
    # Tenuta per compatibilità, ma non usata se c'è Live Energy
