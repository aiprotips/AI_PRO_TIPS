...
from .repo import (
    ...
    schedule_cancel_by_short_id, schedule_cancel_all_today,
...
class CommandsLoop:
    ...
    def handle_update(self, upd: Dict[str, Any]):
        ...
        if low.startswith("/start"):
            self._reply(chat_id, "Benvenuto! Admin 360° attivo. Usa /help per i comandi.")
            return
        ...
        if low.startswith("/status"):
            ...
            q = [s for s in sched if s["status"] == "QUEUED"]
            sent = [s for s in sched if s["status"] == "SENT"]
            nxt = q[0]["send_at"].strftime("%H:%M") if q else "—"
            self._reply(chat_id,
                "<b>Stato di oggi</b>\n"
                f"In coda: <b>{len(q)}</b>\n"
                f"Inviate: <b>{len(sent)}</b>\n"
                f"Prossimo invio: <b>{nxt}</b>"
            )
            return
        ...
        if low.startswith("/preview"):
            ...
            if len(parts) >= 2 and parts[1].isdigit():
                ...
                if not rec:
                    self._reply(chat_id, "ID non trovato.")
                    return
                payload = rec["payload"]
                when = rec["send_at"].strftime("%H:%M")
                self._reply(chat_id, f"ID <b>{sid}</b> — invio: <b>{when}</b>\n\n{payload}")
                return
            else:
                sched = schedule_get_today()
                if not sched:
                    self._reply(chat_id, "Nessuna schedina pianificata oggi.")
                    return
                out = []
                for r in sched:
        ...
        if low.startswith("/publish"):
            parts = text.split()
            if len(parts) < 2:
                self._reply(chat_id, "Uso: /publish ID")
                return
            sid = parts[1].strip()
            rec = schedule_get_by_short_id(sid)
            if not rec or rec["status"] != "QUEUED":
                self._reply(chat_id, "ID non trovato o non in coda.")
                return
            # pubblica subito
            self.tg.send_message(rec["payload"])
            from .repo import schedule_mark_sent
            schedule_mark_sent(rec["id"])
            self._reply(chat_id, f"✅ Pubblicata {sid}.")
            return
        ...
        if low.startswith("/resched"):
            parts = text.split()
            if len(parts) < 3:
                self._reply(chat_id, "Uso: /resched ID HH:MM")
                return
            ...
            try:
                ...
                with get_session() as s:
                    s.execute(
                        text("UPDATE scheduled_messages SET send_at=TIMESTAMP(CONCAT(DATE(send_at),' ', :hm)) WHERE id=:i"),
        ...
        # --- Cancel singola / tutte ---
        if low.startswith("/cancel_all"):
            n = schedule_cancel_all_today()
            self._reply(chat_id, f"🛑 Cancellate <b>{n}</b> schedine in coda oggi.")
            return

        if low.startswith("/cancel "):
            parts = text.split()
            if len(parts) < 2:
                self._reply(chat_id, "Uso: /cancel ID")
                return
            sid = parts[1].strip()
            n = schedule_cancel_by_short_id(sid)
            self._reply(chat_id, "✅ Annullata." if n > 0 else "❌ ID non trovato o già inviata.")
            return
        ...
        if low.startswith("/gen"):
            ...
            # /gen --dry => anteprima senza scrivere in coda
            if "--dry" in parts:
                ...
                for _ in range(self.cfg.DAILY_PLAN["value_singles"]):
                    ...
                    if sgl:
                        previews.append(render_value_single(sgl["home"], sgl["away"], sgl["market"], float(sgl["odds"]), sgl["kickoff_local"], "https://t.me/AIProTips"))
                ...
                for conf in self.cfg.DAILY_PLAN["combos"]:
                    ...
                    if cmb:
                        ...
                        for c in cmb:
                            total *= float(c["odds"])
                            block.append({"home": c["home"], "away": c["away"], "pick": c["market"], "odds": float(c["odds"])})
                        previews.append(render_multipla(block, float(total), cmb[0]["kickoff_local"], "https://t.me/AIProTips"))

                self._reply(chat_id, "<b>Anteprima (dry)</b>\n\n" + ("\n\n——————————\n\n".join(previews) if previews else "Niente da generare."))
                return

            # /gen value  (accoda una singola ad hoc)
            if len(parts) >= 2 and parts[1] == "value":
                today = now_tz(self.cfg.TZ).strftime("%Y-%m-%d")
                used = set()
                sgl = build_value_single(self.auto.api, today, self.cfg, used)
                if not sgl:
                    self._reply(chat_id, "Nessuna singola disponibile.")
                    return
                # Non calcoliamo send_at in /gen value: pubblica subito in coda con send_at NOW
                msg_html = render_value_single(sgl["home"], sgl["away"], sgl["market"], float(sgl["odds"]), sgl["kickoff_local"], "https://t.me/AIProTips")
                from .repo import schedule_enqueue
                from datetime import datetime
                short_id = "V" + str(random.randint(10000, 99999))[1:]  # prefisso opzionale
                schedule_enqueue(short_id, "value", msg_html, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"))
                self._reply(chat_id, f"Accodata singola (ID {short_id}).")
                return

            # /gen combo N  (accoda multipla ad hoc)
            if len(parts) >= 3 and parts[1] == "combo":
                try:
                    n = int(parts[2])
                except Exception:
                    self._reply(chat_id, "Uso: /gen combo N  (N=2,3,4,5,8..12)")
                    return
                today = now_tz(self.cfg.TZ).strftime("%Y-%m-%d")
                used = set()
                # Range default: 2 -> (1.30–1.50), 3/4/5 -> (1.20–1.50), >=8 -> (1.10–1.36)
                ...
                if not cmb:
                    self._reply(chat_id, "Nessuna multipla disponibile.")
                    return
                total = 1.0
                block = []
                for c in cmb:
                    total *= float(c["odds"])
                    block.append({"home": c["home"], "away": c["away"], "pick": c["market"], "odds": float(c["odds"])})
                msg_html = render_multipla(block, float(total), cmb[0]["kickoff_local"], "https://t.me/AIProTips")
                from .repo import schedule_enqueue
                from datetime import datetime
                short_id = "C" + str(random.randint(10000, 99999))[1:]
                schedule_enqueue(short_id, "combo", msg_html, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"))
                self._reply(chat_id, f"Accodata multipla x{n} (ID {short_id}).")
                return

            # /gen  (rigenera piano del giorno)
            kv_set(self.auto._planned_key(), "")  # reset marker
            self.auto.run_daily_planner(force=True)
            self._reply(chat_id, "🔧 Pianificazione giornaliera rigenerata.")
            return

        # --- Check today (match whitelisted) ---
        if low.startswith("/check_today"):
            today = now_tz(self.cfg.TZ).strftime("%Y-%m-%d")
            fixtures = fixtures_allowed_today(self.auto.api, today, self.cfg)
            if not fixtures:
                self._reply(chat_id, "Oggi non ci sono match whitelisted (08:00–24:00).")
                return
            out = []
        ...
        # --- Check picks (lista del giorno + eventuale snapshot quote correnti) ---
        if low.startswith("/check_picks"):
            ...
            for rec in arr[:15]:
                ...
                if not fx:
                    out.append(f"• {rec.get('home','?')}–{rec.get('away','?')} | {rec.get('pick','?')} | fixture {rec.get('fixture_id','?')} n/d")
                    continue
                try:
                    odds = self.auto.api.odds_by_fixture(rec["fixture_id"])
                    mk = self.auto.api.parse_markets_bet365(odds)
        ...
        if low.startswith("/leagues"):
            out = "\n".join(sorted([f"• {c} — {n}" for (c, n) in self.cfg.ALLOWED_COMP_NAMES]))
            self._reply(chat_id, "<b>Whitelist leghe (country/name)</b>\n" + out)
            return

        # --- Dup/Where: optional debug (placeholders) ---
        if low.startswith("/dup"):
            self._reply(chat_id, "Verifica duplicati: OK (nessuna duplicazione nel planner 08:00).")
            return

        if low.startswith("/where"):
            parts = text.split()
            if len(parts) < 2:
                self._reply(chat_id, "Uso: /where FIXTURE_ID")
                return
            fxid = parts[1].strip()
            self._reply(chat_id, f"FIXTURE_ID {fxid}: presente in pianificazione odierna (se generata).")
            return
