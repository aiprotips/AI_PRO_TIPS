...
from .repo import kv_get, kv_set, create_betslip, add_selection, schedule_enqueue, schedule_due_now, schedule_mark_sent
...
class Autopilot:
    ...
    def run_daily_planner(self, *, force: bool=False) -> int:
        ...
        planned_ids = []; used_fixtures=set(); today = now_local.strftime("%Y-%m-%d")
        ...
        for _ in range(int(self.cfg.DAILY_PLAN["value_singles"])):
            ...
            short_id = short_id5(); code = now_local.strftime("%d%H%M") + "-" + str(random.randint(100,999))
            bid = create_betslip(code, round(float(single["odds"]),2), 1, short_id)
            add_selection(bid, single["fixture_id"], single["league_id"], (single["start_time"] or "")[:19], single["home"], single["away"], single["market"], single["pick"], float(single["odds"]))
            self._story_add_match(single["home"], single["away"]); self._picks_add(single["home"], single["away"], single["market"], single["fixture_id"])
            msg_html = render_value_single(single["home"], single["away"], single["market"], float(single["odds"]), single["kickoff_local"], "https://t.me/AIProTips")
            send_at_utc = calc_send_at_for_combo([single], self.cfg.TZ)
            schedule_enqueue(short_id, "value", msg_html, send_at_utc.strftime("%Y-%m-%d %H:%M:%S")); planned_ids.append(short_id)
        ...
        for spec in self.cfg.DAILY_PLAN["combos"]:
            ...
            total=1.0; block=[]
            for c in combo: total*=float(c["odds"]); block.append({"home":c["home"],"away":c["away"],"pick":c["market"],"odds":float(c["odds"])})
            short_id = short_id5(); code = now_local.strftime("%d%H%M") + "-" + str(random.randint(100,999))
            bid = create_betslip(code, round(total,2), len(combo), short_id)
            for c in combo:
                add_selection(bid, c["fixture_id"], c["league_id"], (c["start_time"] or "")[:19], c["home"], c["away"], c["market"], c["pick"], float(c["odds"]))
                self._story_add_match(c["home"], c["away"]); self._picks_add(c["home"], c["away"], c["market"], c["fixture_id"])
            msg_html = render_multipla(block, float(total), combo[0]["kickoff_local"], "https://t.me/AIProTips")
            send_at_utc = calc_send_at_for_combo(combo, self.cfg.TZ)
            schedule_enqueue(short_id, "combo", msg_html, send_at_utc.strftime("%Y-%m-%d %H:%M:%S")); planned_ids.append(short_id)

        kv_set(self._planned_key(), json.dumps(planned_ids, ensure_ascii=False))
        self.tg.notify_admin(f"Planner 08:00: generate & enqueue {len(planned_ids)} schedine." if planned_ids else "Planner 08:00: nessuna schedina generata.")
        return len(planned_ids)

    def process_scheduled_queue(self) -> int:
        due = schedule_due_now(limit=10); count=0
        for rec in due:
            try:
                self.tg.send_message(rec["payload"]); 
                schedule_mark_sent(rec["id"]); count+=1
            except Exception as e:
                from .repo import log_error; log_error("scheduler_send", f"{e}")
        return count
    ...
    def post_stat_flash(self):
        try: arr=json.loads(kv_get(self._picks_key()) or "[]")
        except Exception: arr=[]
        if not arr: return
        pick_rec=random.choice(arr); fx=self.api.fixture_by_id(pick_rec.get("fixture_id",0))
        if not fx: return
        try:
            ...
            line=None
        if line: self.tg.send_message(render_stat_flash(pick_rec.get("home","Home"), pick_rec.get("away","Away"), line_override=line))
